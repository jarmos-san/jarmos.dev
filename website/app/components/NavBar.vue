...
<template>
  <nav
    class="flex items-center justify-between px-8 pt-10 md:px-16 lg:px-28 xl:px-56"
  >
    <!-- Logo -->
    <div>
    ...
    <div v-if="isDesktop">
      <div
        class="flex items-center gap-1.5 rounded-full border border-white/10 bg-white/3 p-1.5 backdrop-blur-md"
      >
        <NuxtLink
          ...
          :class="[
            'text-md rounded-full px-4 py-2 font-medium transition-all duration-200',
            route.path === link.href
              ? 'bg-white/10 text-green-200 shadow-sm'
              : 'text-[#ecf8ff]/80 hover:bg-white/10 hover:text-green-200 hover:shadow-sm',
          ]"
        >
          {{ link.label }}
        </NuxtLink>
    ...
    <DrawerRoot v-else v-model:open="open" swipe-direction="right">
      ...
      >
        <Icon name="material-symbols:menu" size="1.75rem" class="bg-white" />
      </DrawerTrigger>
      ...
      <DrawerPortal>
        ...
        <DrawerContent
          class="content fixed top-0 right-0 z-100 flex h-full w-full max-w-sm flex-col rounded-l-2xl bg-[#162028] shadow-[-4px_0_24px_rgba(0,0,0,0.4)] outline-none"
        >
          <!-- Close button, nav links, social links — evenly spaced -->
          <div
            class="flex flex-1 flex-col items-center justify-between px-6 py-8"
          >
            <DrawerClose
            ...
            <div class="flex flex-col items-center gap-5">
              <NuxtLink
                ...
                :class="[
                  ...
                  route.path === link.href
                    ? '-ml-4.5 border-l-2 border-green-200 pl-4 text-green-200'
                    : 'text-[#ecf8ff]',
                ]"
                @click="open = false"
              >
            ...
            <div class="flex justify-center gap-2">
              <a
                ...
                class="rounded-lg p-2 text-[#ecf8ff]/70 transition-colors duration-200 hover:bg-white/10 hover:text-green-200"
              >
